...
OUTPUT_DIR = DATA_ROOT / "outputs" / "multimodal_concat_baseline"
...
class MultimodalConcatModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.us_encoder = UltrasoundEncoder(us_model_name, pretrained=True, feat_dim=feat_dim)
        self.path_encoder = PathologyEncoderMIL(path_model_name, pretrained=True, feat_dim=feat_dim, attn_dim=128)
    ...
    def forward(self, us_img, path_bag, bag_mask):
        ...
        fused = torch.cat([us_feat, path_feat], dim=1)
...
def train_one_epoch(model, loader, optimizer, criterion, device):
    ...
    for batch in pbar:
        ...
        optimizer.zero_grad()
        logits, _ = model(us_img, path_bag, bag_mask)
...
def evaluate(model, loader, criterion, device):
    ...
    for batch in pbar:
        ...
        logits, attn = model(us_img, path_bag, bag_mask)
        loss = criterion(logits, labels)
...
def main():
    ...
    print("=" * 70)
    print("MULTIMODAL CONCAT BASELINE")
    print("=" * 70)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_metrics["auroc"] > best_auroc:
            ...
            ckpt_path = CHECKPOINT_DIR / f"best_fold{FOLD}_multimodal_concat.pt"
            torch.save({
                "epoch": epoch,
            ...
            pred_path = LOG_DIR / f"best_val_predictions_fold{FOLD}_multimodal_concat.csv"
            pd.DataFrame(val_preds).to_csv(pred_path, index=False)

            print(f"Saved new best checkpoint to: {ckpt_path}")

    history_df = pd.DataFrame(history)
    history_csv = LOG_DIR / f"history_fold{FOLD}_multimodal_concat.csv"
    ...
    summary_path = LOG_DIR / f"summary_fold{FOLD}_multimodal_concat.json"
