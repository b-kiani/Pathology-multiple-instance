...
TRAIN_SCRIPT = DATA_ROOT / "train_multimodal_gated.py"
...
def run_fold(fold):
    print("\n" + "=" * 80)
    print(f"RUNNING MULTIMODAL GATED FOLD {fold}")
    print("=" * 80)
...
def collect_results():
    ...
    for fold in FOLDS:
        ...
        history_path = LOG_DIR / f"history_fold{fold}_multimodal_gated.csv"
        ...
        row = {
            ...
            "best_val_mcc": summary.get("best_val_mcc"),
            "best_val_sensitivity": summary.get("best_val_sensitivity"),
            "best_val_specificity": summary.get("best_val_specificity"),
            "best_val_mean_gate_pathology_weight": summary.get("best_val_mean_gate_pathology_weight"),
        ...
        if history_path.exists():
            ...
            row["best_val_loss"] = best_row.get("val_loss")

        rows.append(row)

    df = pd.DataFrame(rows).sort_values("fold").reset_index(drop=True)

    out_csv = LOG_DIR / "five_fold_summary_multimodal_gated.csv"
    ...
    metric_cols = [
        ...
        "best_val_mean_gate_pathology_weight",
    ]
    ...
    out_json = LOG_DIR / "five_fold_aggregate_multimodal_gated.json"

    with open(out_json, "w") as f:
        json.dump(aggregate, f, indent=2)

    print("\n" + "=" * 80)
    print("MULTIMODAL GATED 5-FOLD SUMMARY")
    print("=" * 80)
